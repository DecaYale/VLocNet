...
import sys
sys.path.insert(0, '../')
from utils.quaternion_math import calc_vos_simple, calc_vos_safe
...
class MF(data.Dataset):
    ...
    def get_indices(self, index):
        ...
        # updated by dy to add recur_pose
        assert(self.steps > 2)
        # offsets -= offsets[len(offsets) // 2
        # offsets -= offsets[len(offsets) // 2+1]
        offsets -= offsets[-1]
    ...
    def __getitem__(self, index):
        ...
        if False and self.include_vos:  # by dy
