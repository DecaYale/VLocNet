...
def qmult(q1, q2):
  """
  Multiply 2 quaternions
  :param q1: Tensor N x T x 4
  :param q2: Tensor N x T x 4
  :return: quaternion product, Tensor N x T x 4
  """
  q1s, q1v = q1[:,...,:1], q1[:,..., 1:]
  q2s, q2v = q2[:,..., :1], q2[:, ..., 1:]
...
if __name__ == '__main__':
    a = np.quaternion(1, 2, 3, 4)
    a_u = a/a.norm()
    # a = a/np.sqrt(a.norm())
    b = np.quaternion(2, 3, 2, 5)
    b_u = b/b.norm()
    print(("a=", a))
    print(("a.norm()=", a.norm()))
    print(("a/a.norm()= ", a_u))
