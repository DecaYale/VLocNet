...
class Criterion(nn.Module):
    ...
    def forward(self, xq_odom, xq_global, xq_gt):
        '''
            xq_odom: Nx7
            xq_global, xq_gt: NxTx7 , T=2 for now ?
        '''
        ...
        x_odom = xq_odom[:, :3]
        q_odom = F.normalize(xq_odom[:, 3:])

        x_global = xq_global[:, 1, :3]
        q_global = F.normalize(xq_global[:, 1, 3:])
        x_global_p = xq_global[:, 0, :3]
        q_global_p = xq_global[:, 0, 3:]
